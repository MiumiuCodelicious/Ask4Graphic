...
def main(argv):
    print_attributes = 0
    print_entities = 1
    print_both_entities_attributes = 2
    no_print = 3
    
    printing_choice = no_print
    '''
     The following are for given a new query -----------------
    '''
    print "You asked a question: ", argv
    parseTree = getParseTree(argv)
    entityChunks, attribute_matrix = chunkQuery(0, parseTree, printing_choice)   
    ...
    print formulateDescription(entities, XYClass, IMClass, FXlist)
...
def formulateDescription(entities, XYClass, IMClass, FXlist):
    # Formulate the natural language description of wanted graph. 
    response = "\nYour want a graphic "
    X = "\" and \"".join( [entities[en] for en, label in enumerate(XYClass) if label == "X"] )
    Y = " ".join( [entities[en] for en, label in enumerate(XYClass) if label == "Y"] )
    Message = "just show data."
    
    if (IMClass == "Get-Rank" or IMClass == "Rank-All" or IMClass == "Relative-Difference"):
        if len(X) > 2: response += "comparing \"" + X + "\""
        if len(Y) > 2: response += " according to " + Y
        if (IMClass == "Rank-All"): Message = "an overall ranking of " + X
        elif (IMClass == "Get-Rank"): Message = "the specific ranking of " + "\" and \"".join(FXlist) + "\""
        elif (IMClass == "Relative-Difference"): Message = "the difference between " + "\" and \"".join(FXlist) + "\""
    elif (IMClass == "Trend"):
        if len(Y) > 2: response += "showing the trend of \"" + Y + "\""
        if len(X) > 2: response += " along \"" + X + "\""
        Message = "a trend"
    elif (IMClass == "Maximum-Minimum-Multiple"):
        if len(X) > 2: response += "listing \"" + X + "\""
        if len(Y) > 2: response += " according to " + Y
        Message = "max or min"
    elif (IMClass == "Maximum-Minimum-Single"):
        if len(X) > 2: response += "showing \"" + X + "\""
        if len(Y) > 2: response += " according to " + Y
        Message = "max or min"
    elif (IMClass == "General-Single" or IMClass == "General-Multiple"):
        if len(X) > 2: response += "about \"" + X + "\""
        if len(Y) > 2: response += " and " + Y

    response += ".\n"
    
    response += "Ideal graphic X-axis should describe: " + X + "\nIdeal graphic Y-axis should describe: " + Y + "\nIdeal graphic should convey: " + Message
    return response + "."
